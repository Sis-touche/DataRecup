...
// ============================================================================
// FONCTION UTILITAIRE DE VUE INTERFACE (EXPORT + VIDAGE CLASSIQUE)
// ============================================================================
...
Future<void> exporterEtEventuellementVider(
  ...
  // Vérifie s'il y a des fiches avant tout
  ...
  final file = await dbHelper.exporterVersExcelSansPartage();
  if (file == null) {
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Export annulé ou permission refusée.'),
          backgroundColor: Colors.orange,
        ),
      );
    }
  ...
  // ── Partage natif (feuille Android/iOS) ──────────────────────────────────
  // L'utilisateur choisit lui-même : Fichiers, Drive, WhatsApp, email...
  await Share.shareXFiles(
    [XFile(file.path)],
    text: 'Export enquête VIH – ${file.path.split('/').last}',
    subject: 'Export données enquête',
  );

  // ── Demander si vider la base ─────────────────────────────────────────────
  if (demanderVidage && context.mounted) {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'Voulez-vous supprimer toutes les fiches après l\'export ?'),
        actions: [
          TextButton(
